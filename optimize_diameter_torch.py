...
def loss_fun(wv_sweep, device, nG=40, 
...
def optimize_diameter_torch(wv_sweep, device, nG=40, 
          theta_start=0, theta_end=80, n_theta=10, theta_sweep=True, 
    ...
    if theta_sweep == False: use theta_start as the angle
    """
    diameter = torch.tensor(diameter, requires_grad=True)

    optimizer = torch.optim.SGD([diameter], lr=weight)

    print("Initial loss:", loss_fun(wv_sweep, device, nG, 
          theta_start, theta_end, n_theta, theta_sweep, 
          Nx, Ny, Np, structure, diameter, plot=plot))

    for i in range(epoch):
        optimizer.zero_grad()
        loss = loss_fun(wv_sweep, device, nG, 
                        theta_start, theta_end, n_theta, theta_sweep, 
        ...
        if i % print_every == 0:
            ...
            print("Thicknesses:", diameter.detach().numpy())

    print("Final loss:", loss_fun(wv_sweep, device, nG, 
                                  theta_start, theta_end, n_theta, theta_sweep, 
                                  Nx, Ny, Np, structure, diameter, plot=plot))
    
    return diameter.detach().numpy()